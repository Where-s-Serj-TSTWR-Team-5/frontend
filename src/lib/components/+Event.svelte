...
>
  <div class="relative h-44 overflow-hidden">
    ...
    <div class="absolute bottom-3 right-3 flex flex-row gap-2 items-end">
      ...
      <button
        type="button"
        onclick={(e) => { e.stopPropagation(); e.preventDefault(); openDeleteModal(); }}
  ...
  <div class="p-5 flex flex-col gap-4">
    <div>
      <h3 class="font-serif text-xl font-bold text-stone-900 leading-tight">{event.title}</h3>
      <p class="text-sm text-stone-600 italic mt-1 line-clamp-2">{event.subtitle}</p>
    </div>
