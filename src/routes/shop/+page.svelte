<script lang="ts">
  // Import icons needed for the page content, NOT the main navigation
  import { Trophy, Leaf, Calendar, Clock } from 'lucide-svelte';

  // --- Mock Data --- (Kept local to this page)
  const PLANTS = [
    { 
      id: 1, 
      name: 'Mango', 
...
<div>
  ...
  <header class="flex justify-between items-center px-6 pt-8 pb-6 md:pb-10 md:pt-4">
    <div>
    ...
    <div class="flex items-center space-x-2 bg-stone-200 px-3 py-1 md:px-5 md:py-2 rounded-full transition hover:bg-stone-300 cursor-pointer">
  ...
  <main class="grid grid-cols-1 lg:grid-cols-12 gap-8 px-6 pb-12">
    ...
    <section class="space-y-4 lg:col-span-8">
      ...
      <div class="flex overflow-x-auto pb-4 md:pb-0 md:grid md:grid-cols-2 xl:grid-cols-3 gap-3 md:overflow-visible scrollbar-hide">
        {#each PLANTS as plant}
          <a href="/plants/{plant.id}" class="flex-shrink-0 w-auto md:w-full bg-white rounded-full md:rounded-xl px-4 py-2 md:p-3 flex items-center space-x-3 shadow-sm border cursor-pointer hover:shadow-md active:scale-95 transition {statusBorders[plant.status]}">
    ...
    <section class="space-y-4 lg:col-span-4">
      ...
      <div class="space-y-4">
        {#each EVENTS.slice(0, 2) as event}
          <a href="/events/{event.id}" class="w-full text-left bg-white rounded-2xl overflow-hidden shadow-md border border-stone-100 cursor-pointer hover:shadow-lg active:scale-[0.98] transition group">
      ...
      <a href="/events" class="hidden lg:block w-full py-2 text-center text-sm font-bold text-stone-500 hover:text-green-700 transition">
        View all events →
      </a>
    </section>
