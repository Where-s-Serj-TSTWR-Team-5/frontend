<script>
	// Import necessary Lucide icons for rich detail presentation
	import {
		ChevronLeft,
		Calendar,
		Clock,
	...
	// Formats the date string (e.g., 'Tuesday, December 16, 2025')
	const formatDate = (isoString) => {
		if (!isoString) return "TBD";
		try {
			// Date part only: e.g., "Dec 16, 2025"
	...
	// Formats the time string (e.g., '1:00 PM')
	const formatTime = (isoString) => {
		if (!isoString) return "";
		try {
			// Time part only: e.g., "1:00 PM"
			return new Intl.DateTimeFormat("en-US", {
				hour: "numeric",
				minute: "2-digit",
				hour12: true,
	...
	// Apply formatting to event properties
	const displayDate = formatDate(event.date || event.startAt);
	const displayStartTime = formatTime(event.startAt);
	const displayEndTime = formatTime(event.endAt);

	// Helper function for dynamic capacity status
	const getParticipantStatus = () => {
		if (event.currentParticipants >= event.maxParticipants) {
			return { text: "Fully Booked", color: "text-red-700 bg-red-100 border-red-200" };
		} else if (event.currentParticipants / event.maxParticipants > 0.75) {
			return { text: "Almost Full", color: "text-amber-700 bg-amber-100 border-amber-200" };
		}
		return { text: "Seats Available", color: "text-green-700 bg-green-100 border-green-200" };
	};
...
<div
	class="flex flex-col h-full bg-stone-50 overflow-y-auto animate-in slide-in-from-right duration-300 pb-8"
>
	<div class="relative h-72 md:h-96 w-full shrink-0">
		...
		<button
			...
			class="absolute top-8 left-6 z-20 bg-black/50 text-white p-3 rounded-full shadow-xl hover:bg-black/70 transition duration-300 ease-in-out"
			aria-label="Go back"
		>
			...
			>
				...
				<div class="grid grid-cols-1 sm:grid-cols-2 gap-4 border-t border-stone-100 pt-5">
					...
					<div class="flex items-center space-x-3 p-3 bg-stone-50 rounded-xl col-span-full">
						...
						<div>
							<span class="text-xs font-semibold uppercase text-stone-500">Location</span>
							<p class="font-medium text-stone-800">{event.location || "Online / TBD"}</p>
						</div>
					</div>
				</div>
			</div>
