<script>
  ...
  const handleSubmit = async () => {
    ...
    if (
      ...
      formError =
        "Please fill out all required fields (Title, Date, Time, Location).";
      isSubmitting = false;
      return;
    }
    ...
    // Simulate a successful save delay
...
<div
  id="modal-backdrop"
  class="relative w-full h-full p-4 sm:p-8 pb-[5rem] sm:pb-8"
...
>
  ...
  >
    ...
    <form
      ...
      class="p-6 space-y-8 flex-grow overflow-y-auto"
    >
      <div class="space-y-4">
        <h3
          class="text-xl font-semibold text-stone-700 flex items-center gap-2"
        >
        ...
        <div class="flex flex-col">
          <label
            for="category"
            class="font-medium text-stone-600 mb-1 flex items-center gap-1"
          >
      ...
      <div class="space-y-4 pt-4 border-t border-stone-100">
        ...
        <div class="flex flex-col">
          <label for="imageUrl" class="font-medium text-stone-600 mb-1"
            >Image URL</label
          >
          <input
            id="imageUrl"
      ...
      <div class="pt-6 shrink-0">
        {#if formError}
          <p class="text-sm text-center text-red-500 mb-3">{formError}</p>
        {/if}
        <button
          type="submit"
        ...
        >
          {#if isSubmitting}
            <div
              class="w-5 h-5 border-2 border-t-2 border-white border-opacity-70 rounded-full animate-spin"
            ></div>
            Submitting...
          {:else}
            Create Event
